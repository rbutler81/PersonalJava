...
public class ALMD implements CSVWriter{
	...
	public ALMD(String scope, String name, String description, ALMDAttributes attributes, Map<String, String> langDesc){
	...
	public List<String[]> toCSV(List<?> l) {
		...
		r.add(new String[] {"remark", "\"Date = " + new SimpleDateFormat("EEE MMM dd HH:mm:ss yyyy").format(new Date()) + "\""});
		...
		if (l.size() > 0){
			...
			a = a.stream()
				.sorted(Comparator.comparing(ALMD::getScope).thenComparing(ALMD::getName))
			...
			for (ALMD e : a){
				...
				if (firstLoop || !lastScope.equals(e.getScope())){
					...
					r.add(new String[] {"TYPE","SCOPE","NAME","DESCRIPTION","DATATYPE","SPECIFIER","ATTRIBUTES"});
				}
				
				r.add(new String[] {"TAG", e.getScope(), e.getName(), "\"\"", "\"ALARM_DIGITAL\"", "\"\"", e.getAttributes().toString()});
				
				if (!e.getDescription().equals("")){
					r.add(new String[] {"ALMMSG:en-US", e.getScope(), e.getName(), "\"" + e.getDescription() + "\"", "", "\"AM\""});
				}
	...
	private void genLanguages(){
		...
		if (langDesc.entrySet().size() > 0) {	
			List<String> languageList = langDesc.entrySet().stream()
								.filter(e -> e.getKey() != null)
								.sorted(Map.Entry.comparingByKey())
								.map(e -> e.getKey() )
								.filter(e -> !e.equals("en-US") && !e.equals(""))
								.filter(e -> e != null)
								.collect(Collectors.toList());
	...
	public static List<ALMD> findALMDS(List<String[]> list) {
		...
		//Filter the list down to only ALMD instructions and group the associated lines into a map
		Map<String, CSVLines> m = new HashMap<String, CSVLines>();
		list.stream()
				.filter(p -> Array.getLength(p) > 5)
				.filter(p -> !p[1].endsWith("AOI"))
				.filter(p -> (p[4].contains("ALARM_DIGITAL") || p[0].contains("ALMMSG:")))
				.forEach(p -> {
		...
		//Strip off extra CSV container data and merge any columns that were split because there was a "," in the "description" field
		...
		List<ALMD> lo = n.entrySet().stream()
				.map(e -> {
					...
					for (int x = 2; x <= e.getValue().getNumberOfLines(); x++){
						...
						if (lang.equals("en-US")) desc = _desc;
					}
