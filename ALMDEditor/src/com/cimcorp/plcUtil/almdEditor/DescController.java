...
public class DescController {
	...
	@FXML
	private TextArea txtLangDesc;
	@FXML
	private TextField txtNewLang;
	@FXML
	private TextField txtFixedWidth;
	@FXML
	private TextField txtNumOfDig;
	@FXML
	private TextField txtDecPlaces;
	...
	@FXML
	private ComboBox<String> comboLeftFill;

	@FXML
	private Label lblNumOfDigits;
	@FXML
	private Label lblDecPlaces;
	@FXML
	private Label lblCharacters;
	@FXML
	private Label lblLeftFill;

	@FXML
	private CheckBox chkFixedWidth;
	@FXML
	private CheckBox chkBool;
	@FXML
	...
	@FXML
	private ListView<String> lstLang;
	...
	public void initialize() {
		...
		ObservableList<String> comboItems = FXCollections.observableArrayList("Alarm Name", "Condition Name",
				"Input Value", "Limit Value", "Severity", "Tag 1", "Tag 2", "Tag 3", "Tag 4");
			...
			if (newVal.equals("Alarm Name") || newVal.equals("Condition Name")) {
				...
				if (newVal.equals("Severity")) {
					...
					if (chkBool.isSelected())
						boolSelected();
					else if (chkString.isSelected())
						stringSelected();
					else if (chkReal.isSelected())
						realSelected();
				} else {
	...
	void onOKBtn(ActionEvent event) {
		if (langDescMap.containsKey("en-US"))
			mainController.getTxtDescription().setText(langDescMap.get("en-US"));
		else
			mainController.getTxtDescription().setText("");
		if (langOList.size() == 0) {
			langDescMap.clear();
			langOList.clear();
		}
	...
	void onAddDescBtn(ActionEvent event) {
		if (langOList.size() > 0) {
			if (comboSel.getSelectionModel().getSelectedItem().equals("Alarm Name")
				...
				txtLangDesc.setText(
						txtLangDesc.getText() + digitsWithDecimals(comboSel.getSelectionModel().getSelectedItem()));
			else if (comboSel.getSelectionModel().getSelectedItem().equals("Severity"))
				txtLangDesc.setText(
						txtLangDesc.getText() + digitsNoDecimals(comboSel.getSelectionModel().getSelectedItem()));
			else {
				if (chkBool.isSelected())
					txtLangDesc.setText(
							txtLangDesc.getText() + digitsNoDecimals(comboSel.getSelectionModel().getSelectedItem()));
					...
					txtLangDesc.setText(
							txtLangDesc.getText() + fixedWidth(comboSel.getSelectionModel().getSelectedItem()));
				else
					txtLangDesc.setText(
							txtLangDesc.getText() + digitsWithDecimals(comboSel.getSelectionModel().getSelectedItem()));
			}
	...
	@FXML
	void onChkBool(ActionEvent event) {
		boolSelected();
	}

	@FXML
	void onChkString(ActionEvent event) {
		stringSelected();
	}

	@FXML
	void onChkReal(ActionEvent event) {
		realSelected();
	}
	...
	private void groupTwoVis() {
		txtFixedWidth.setDisable(true);
		if (comboSel.getSelectionModel().getSelectedItem().contains("Tag"))
			chkFixedWidth.setDisable(false);
		else
	...
	private String leftFill() {
		if (comboLeftFill.getSelectionModel().getSelectedItem().equals("None"))
			return "NOFILL";
		else if (comboLeftFill.getSelectionModel().getSelectedItem().equals("Spaces"))
			return "SPACEFILL";
		else
			return "ZEROFILL";
	}
